 ...
 * Description: Easy to set up and configure, lite-weight (.min is ~13k) full-featured (and responsive) content slider (or fade). Works on images (see demo), as well as non-images (with minor adjustments).
 * Requires: 1.7.x - Designed and developed using 1.7.x. (Not sure if it works with any earlier jQ releases but you're welcome to try.)
 * Author: Mark "Chief Alchemist" Simchock (http://ChiefAlchemist.com)
 * Copyright: Copyright 2012 Alchemy United (AlchemyUnited.com)
...
(function($) {
	...
	$.fn.ezContentSlider = function(options) { 
		...
		if ( oEzcs.cMainSlidesList.length > 0 ) {
			...
			// let's plan ahead a bit...once the parent <li> is display:none, assessing children can get iffy, so we'll stash the image width, height and aspect ratio now.  
	...
	function ezcsSetupControls($this,oEzcs){
		...
		if ( oEzcs.opts.carouselUse ) {
			...
			oEzcs.cCarouselPagesList.click(function(){	
		...
		cMainPrevSelector.click(function(){
			// should playAuto be stopped?
			if ( oEzcs.opts.playAutoPlaying && oEzcs.opts.mainNextPrevClickStopsPlayAuto ) {
				oEzcs.opts.playAutoPlaying = false;
				clearInterval(oEzcs.autoPlayInterval);
		...
		cControlsSwitchClass.click(function(){
		...
		cControlsDisplayClass.click(function(){
		...
		cControlsThumbsClass.click(function(){
		...
		cControlsCaptionClass.click(function(){
		...
		cControlsShareClass.click(function(){
		...
		oEzcs.cControlsPlayClass.click(function(){
			oEzcs.cControlsPlayClass.toggle();
			if ( oEzcs.opts.playAutoPlaying ) {
				clearInterval(oEzcs.autoPlayInterval);
				oEzcs.opts.playAutoPlaying = false;
	...
	controlsSwitchWrapperClass : '.ezcs-controls-switch-wrapper', // what wraps everything but the switch control? this is toggled by the switch click
	...
	carouselPageSelectedClassName : 'selected', // Note: This setting is simply the class name, the leading period (.) should NOT be included.
	...
	carouselGroupPageOverlap : 0, // how many pages (read: thumbnaiils) from the current group being displayed will be "overlapped" into the next / prev group to be diaplayed? Overlapping can make it easier for user to follow the group paging. 
	...
	carouselPageWidthInt : 68, // just the integert (w/out the px). img width + borders + margins + paddings. you have to do the math yourself (so the plugin doesn't have invest overhead to do so).
	...
	slidesPerSlideDuration : 200, // what "speed" is the slide. should not be greater than playAutoDuration (duh?)
	...
	displayFullStyle : 'fill', // 'fit' else any other value defaults to 'fill'
	...
	displayFullImgHeight100PercentClassName : 'ezcs-height-100-percent', // Note: This setting is simply the class name, the leading period (.) should NOT be included.
 	fullToThisAddClassName : 'ezcs-status-fullscreen', // Note: This setting is simply the class name, the leading period (.) should NOT be included. This value is used in an addClass()
	displayIsReg : true,
